...
first_non_missing <- function(v) {
  if (is.list(v)) v <- vapply(v, function(x) if (length(x)) x[[1]] else NA, FUN.VALUE = NA_character_)
  if (is.character(v)) {
    idx <- which(!is.na(v) & v != "")[1]
  } else {
    idx <- which(!is.na(v))[1]
  }
  if (is.na(idx)) NA else v[[idx]]
}
...
add_readmit_seq <- function(adm_df, readmit_gap_days = 15L, transfer_gap_max = 2L) {
  ...
  adm_df %>%
    ...
    mutate(
      ...
      readmit_seq = cumsum(coalesce(new_episode, 0L))
    ) %>%
    ungroup()
}
